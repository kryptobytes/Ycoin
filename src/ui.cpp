...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) + "\\ycoin.lnk";
}
...
{
    return GetAutostartDir() / boost::filesystem::path("ycoin.desktop");
}
...
void SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
        {
            wxMessageBox(_("Cannot write autostart/ycoin.desktop file"), "ycoin");
            return;
        }
        // Write a ycoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ycoin\n";
...
#endif
#if defined(__WXGTK__) || defined(__WXMAC_OSX__)
    m_checkBoxStartOnSystemStartup->SetLabel(_("&Start ycoin on window system startup"));
    if (!GetBoolArg("-minimizetotray"))
    {
...
CAddressBookDialog::CAddressBookDialog(wxWindow* parent, const wxString& strInitSelected, int nPageIn, bool fDuringSendIn) : CAddressBookDialogBase(parent)
{
    ...
    m_listCtrlReceiving->InsertColumn(1, _("ycoin Address"), wxLIST_FORMAT_LEFT, 350);
...
void CMyTaskBarIcon::Show(bool fShow)
{
    ...
    if (fShow)
    {
        string strTooltip = _("ycoin");
        if (fGenerateBitcoins)
            strTooltip = _("ycoin - Generating");
        if (fGenerateBitcoins && vNodes.empty())
            strTooltip = _("ycoin - (not connected)");
...
#else
            SetIcon(ycoin80_xpm, strTooltip);
#endif
        }
    }
    else
...
wxMenu* CMyTaskBarIcon::CreatePopupMenu()
{
    ...
    pmenu->Append(ID_TASKBAR_RESTORE, _("&Open ycoin"));
    pmenu->Append(ID_TASKBAR_SEND, _("&Send ycoins"));
...
bool CMyApp::OnInit()
{
    ...
    g_locale.AddCatalog("bitcoin"); //ycoin todo
