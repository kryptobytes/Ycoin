...
void PrintConsole(const char* format, ...)
{
    ...
    MyMessageBox(buffer, "ycoin", wxOK | wxICON_EXCLAMATION);
#else
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new ycoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current ycoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <ycoinaddress> <account>\n"
    ...
    if (!isValid)
        throw JSONRPCError(-5, "Invalid ycoin address");
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        BOOST_FOREACH(const PAIRTYPE(string, string)& item, pwalletMain->mapAddressBook)
        {
            ...
            {
                // We're only adding valid ycoin addresses and not ip addresses
                CScript scriptPubKey;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <ycoinaddress> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001");
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <ycoinaddress> [minconf=1]\n"
            "Returns the total amount received by <ycoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toycoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        CScript scriptPubKey;
        if (!scriptPubKey.SetBitcoinAddress(strAddress))
            throw JSONRPCError(-5, string("Invalid ycoin address:")+strAddress);
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <ycoinaddress>\n"
            "Return information about <ycoinaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "ycoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "ycoin is downloading blocks...");
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: ycoin-json-rpc/" << FormatFullVersion() << "\r\n" 
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    {
        string strWhatAmI = "To use ycoind";
...
#else
    if (fUseSSL)
        throw runtime_error("-rpcssl=1, but ycoin compiled without full openssl libraries.");
#endif
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    ip::tcp::iostream stream(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "60000"));
    if (stream.fail())
        throw runtime_error("couldn't connect to server");
#endif
...
int CommandLineRPC(int argc, char *argv[])
{
    ...
    if (strPrint != "")
    {
        ...
        MyMessageBox(strPrint, "ycoin", wxOK);
#else
