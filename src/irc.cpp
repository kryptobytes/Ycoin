...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ycoinTEST\r");
            Send(hSocket, "WHO #ycoinTEST\r");
        } else {
            // randomly join #ycoin00-#ycoin09
            int channel_number = GetRandInt(10);
            Send(hSocket, strprintf("JOIN #ycoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ycoin%02d\r", channel_number).c_str());
        }
